...
3. Change the value to the next meaningful state in the **TUV Scrum for Mobility** process (based on Scrum), for example:
   - `New` → `Approved`
   - `Approved` → `Committed`
   - `Committed` → `Done`
4. Save the change.
...
- If the **State** cannot be changed, the work item is usually not of type **Product Backlog Item**. Open a work item that was clearly created as a Product Backlog Item instead.
- If an **attachment** does not appear after upload, the work item has most likely not been saved yet. Click **Save** again.
- If a **comment** does not show up in the history, it was typed but not confirmed with **Save** or the comment button.
- If a new work item is missing from the backlog, usually the **wrong backlog level** is selected. Make sure the **Product Backlog Items** level is active.
