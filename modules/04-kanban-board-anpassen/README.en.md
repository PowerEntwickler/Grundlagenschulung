...
## ✅ Prerequisites
- [Module 01](../01-grundlagen-arbeitselemente-erzeugen/README.en.md) to [Module 03](../03-import-massenbearbeitung-queries-charts/README.en.md) have been completed
...
6. Add at least one extra custom column that makes waiting or alignment phases visible, for example:
   - `In Review`
   - or `Waiting`
7. Save the change.
...
## Exercise 7 - Use Card styles and Tag colors

### 🎯 Goal
Improve how important information stands out on the board.

Azure DevOps Services offers two different features for this, both configured under **Board settings > Styles**:

- **Card styles** = rules that colorize cards based on field values, for example when a certain tag or state is set.
- **Tag colors** = fixed colors per tag, shown wherever the tag appears.

### 🔧 Steps

**Part A - Card styles**

1. Open **Board settings** in the upper right corner.
2. Switch to **Styles** and then to the **Card styles** tab.
3. Click **Add styling rule**.
4. Give it a meaningful name, such as `Highlight communication`.
5. Define the condition, for example **Tags** `Contains` `communication`.
6. Choose a clearly visible background or text color.
7. Save the rule.

**Part B - Tag colors**

1. In the **Styles** dialog, switch to the **Tag colors** tab.
2. Enable the display of tag colors if such an option is available.
3. Click **Add tag color** and pick a tag such as `reporting`, `risk`, or `communication`.
4. Choose a distinct color from the palette for that tag.
5. Add colors for one or two additional tags.
6. Save the settings.

**Part C - Verify the effect**

1. Return to the board.
2. Verify that:
   - Work items with the defined tag are colored by the **Card styles** rule
   - the **Tag colors** appear on individual cards

### 🟢 Expected result
Important categories or topics can now be recognized much faster on the board, both through card backgrounds (**Card styles**) and through colored tags (**Tag colors**).
...
7. visible highlighting through **Card styles** and **Tag colors**
